...
trait EndpointParser extends LogSupport {
  ...
  private val multiExtractors: MMap[String, JsObject => List[Option[PII]]] = MMap()
    ...
    try {
      ...
      case x: Throwable =>
        try {
          ...
          case _: Throwable =>
            List(RequestParsingIssue(x.getMessage + x.getStackTrace.mkString("\n", "\n", "\n")))
        }
  ...
  protected def extract(request: JsObject): List[Option[PII]] = {
    extractors
      .map { case (extractorName, extractPii) =>
        try {
          extractPii(request)
        } catch {
          case x: Throwable =>
            Some(ExtractorFailure(this.trackerName, extractorName, request, x.getMessage))
        }
      }
      .toList ++
      multiExtractors
        .flatMap { case (multiExtractorName, extractMultiplePii) =>
          try {
            extractMultiplePii(request)
          } catch {
            case x: Throwable =>
              List(Some(ExtractorFailure(this.trackerName, multiExtractorName, request, x.getMessage)))
          }
        }
        .toList
  }

  protected def addExtractor(name: String)(func: JsObject => Option[PII]): Unit = {
    extractors.addOne(name -> func)
  }

  protected def addMultiExtractor(name: String)(func: JsObject => List[Option[PII]]): Unit = {
      ...
      queryString
        ...
        .map { keyValue: String =>
          keyValue.split("=").toList match {
            case Nil =>
              throw new RuntimeException(s"weird key value $keyValue in $queryString")
            case key :: Nil =>
              (key, None)
            case key :: value :: Nil =>
              (key, Some(value))
        ...
        .map { case (key, value) =>
          val jsValue: JsValue =
            value
              .map(_._2)
              .filter(_.nonEmpty)
              .map(elem => JsString(URLDecoder.decode(elem.get, StandardCharsets.UTF_8))) match {
              case Nil =>
                JsNull
              case single :: Nil =>
                single
              case multiple =>
                JsArray(multiple.toVector)
            }
