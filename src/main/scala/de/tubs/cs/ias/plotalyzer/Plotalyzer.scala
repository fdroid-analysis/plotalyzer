...
import de.tubs.cs.ias.plotalyzer.cli.ExperimentCommand
...
object Plotalyzer extends LogSupport {
  ...
  val parser
      : Parser = Parser("plotalyzer", "takes a scala-appanalyzer database and generates json for further processing")
    ...
    .addSubparser(
      Parser("generateOnlySet", "given an app metadata json, filter for apps matching specifications")
    ...
    .addSubparser(
      Parser("tcfDetection", "analyze set of apps for any tcf indicating values")
        ...
        .addSubparser(
          Parser("dynamic", "perform analysis on dynamically collected data")
            .addDefault[ParsingResult => JsValue]("func", generateDynamicTcfSummaryMain)
        )
        .addSubparser(
          Parser("static", "use statically extracted file data")
    ...
    .addSubparser(
      Parser("summary", "high level summary of the experiment")
        .addDefault[ParsingResult => JsValue]("func", generateExperimentSummaryMain)
    )
    .addSubparser(
      Parser("error", "summarize encountered errors")
        .addDefault[ParsingResult => JsValue]("func", generateErrorSummaryMain)
    )
    .addSubparser(
      Parser("failures", "summarize failed analysis and apps")
    ...
    .addSubparser(
      Parser("trafficCollection", "analyze the collected traffic")
        .addOptional("trafficConfig", "c", "config", Some("./resources/trafficCollection/config.json"))
        .addOptional("only", "o", "only", None, "only analyze the provided apps")
    ...
    .addSubparser(
      Parser("consentDialog", "analyze a consent dialog analysis experiment")
        ...
        .addSubparser(
          Parser("spotcheck", "perform an spotcheck of the analysis results")
            .addOptional(
              "include",
              "i",
              "include",
              None,
              "path to a file containing line by line list of to be included apps"
            )
    ...
    .addSubparser(ExperimentCommand.parser)
            ...
            if (categories.isEmpty) {
              if (
                (bestCategory.isEmpty ||
                  bestCategory.get == metaDataObj.fields("bestCategory").asInstanceOf[JsString].value) &&
    ...
    val hostName: Option[TrafficFilter] =
      (
        ...
        case (None, None) =>
          None
        case _ =>
          throw new RuntimeException("-j/--filter-file and -t/--filter-type have to be used together")
      }
  ...
  private def generateTrafficCollectionAnalysisMain(pargs: ParsingResult): JsValue = {
    ...
    JsObject(
      ...
      "trafficTrackingEndpoints" ->
        RequestTrackingEndpointAnalysis(experiment, TrafficCollectionAnalysisConfig.get(confPath), only).toJson
    )
  }
