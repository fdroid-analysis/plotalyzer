...
import de.tubs.cs.ias.plotalyzer.cli.{ExperimentCommand, PrivacyLabelCommand}
...
object Plotalyzer extends LogSupport {
  ...
  val parser: Parser = Parser("plotalyzer", "takes a scala-appanalyzer database and generates json for further processing")
    ...
    .addSubparser(Parser("generateOnlySet", "given an app metadata json, filter for apps matching specifications")
    ...
    .addSubparser(Parser("trafficSummary", "list all the request collected during the experiment")
    ...
    .addSubparser(Parser("trafficCollection", "analyze the collected traffic")
