...
import de.tubs.cs.ias.plotalyzer.utility.color.ColorRepresentation._
...
object CIDE2000 {
  ...
  def calculateDeltaE(lhs: Lab, rhs: Lab): Double = {
    ...
    val h1prime = Math.atan2(b1, a1prime) +
      2 * Math.PI * (if (Math.atan2(b1, a1prime) < 0) 1 else 0)
    val h2prime = Math.atan2(b2, a2prime) +
      2 * Math.PI * (if (Math.atan2(b2, a2prime) < 0) 1 else 0)
    val Hmeanprime =
      if ((Math.abs(h1prime - h2prime) > Math.PI)) (h1prime + h2prime + 2 * Math.PI) / 2
      else (h1prime + h2prime) / 2

    val T = 1.0 - 0.17 * Math.cos(Hmeanprime - Math.PI / 6.0) + 0.24 * Math.cos(2 * Hmeanprime) +
      0.32 * Math.cos(3 * Hmeanprime + Math.PI / 30) -
    ...
    val deltaHprime = 2.0 * Math.sqrt(C1prime * C2prime) * Math.sin(deltahprime / 2.0)
    val SL = 1.0 +
      ((0.015 * (Lmean - 50) * (Lmean - 50)) / Math.sqrt(20 + (Lmean - 50) * (Lmean - 50)))
    val SC = 1.0 + 0.045 * Cmeanprime
    val SH = 1.0 + 0.015 * Cmeanprime * T

    val deltaTheta = (30 * Math.PI / 180) *
      Math
        .exp(-((180 / Math.PI * Hmeanprime - 275) / 25) * ((180 / Math.PI * Hmeanprime - 275) / 25))
