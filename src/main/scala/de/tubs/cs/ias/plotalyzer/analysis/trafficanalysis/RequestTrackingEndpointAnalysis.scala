...
class RequestTrackingEndpointAnalysis(
  ...
  /** All apps from the experiment mapped to all of their interface analyses that were conducted.
    * Only uses apps that never had an unsuccessful interface analysis
    */
  private val appMap: Map[MobileApp, List[InterfaceAnalysis]] = interfaceAnalysis
    .groupBy(_.getApp)
    .filterNot(_._2.exists(_.getInterfaceErrors.nonEmpty))
    .filter(_._2.nonEmpty)
  private val overallSize: Int = appMap.size
  info(s"we have analysis sets for $overallSize different apps, performing traffic analysis...")

  /** Map of the following structure:
    * MobileApp -> List[(TrafficCollection Description, Option[AppReqTrackingEndpointAnalysis])]
    */
  private val analysis
      : Map[MobileApp, List[(String, Option[AppRequestTrackingEndpointAnalysis])]] = {
    val bar = AsciiProgressBar.create("app analysis sets", overallSize.toLong)
    try {
      val fut = Future.sequence {
        appMap.map { case (app, analysis) =>
          Future {
            try {
  ...
  def toJson: JsValue = {
    JsObject(
      ...
      "_overallInterestingRequests" -> JsNumber(this.getOverallInterestingRequests),
      "_addressedByEndpointParser" -> JsNumber(this.getCoveredByEndpoint),
      "_summary" ->
        JsObject(
          getAppRequestAnalysisMap.map { case (descr, analysis) =>
            descr -> generateAnalysisSummary(analysis)
          }
        ),
      "_unaddressed" ->
        AppRequestTrackingEndpointAnalysis.missedEndpointsToJsObject(this.getUnaddressed),
      "errors" ->
        JsObject(
          this
            .getErrors
            .filter(_._2.nonEmpty)
            .map { case (endpoint, errors) =>
      ...
      "apps" ->
        JsObject(
          this
            .analysis
            .map { case (app, analysis) =>
              app.toString ->
                JsObject(
                  analysis
                    .map { case (tcDescr, analysis) =>
                      tcDescr ->
                      (analysis match {
                          case Some(analysis) =>
                            analysis.toJson
  ...
  def getAppRequestAnalysisMap: Map[String, List[AppRequestTrackingEndpointAnalysis]] = {
    val ret: MMap[String, ListBuffer[AppRequestTrackingEndpointAnalysis]] = MMap()
  ...
  def getOverallRequests: Int = getAppRequestAnalyses.map(_.getRequestCount).sum

  /** Request count that were covered by an host from the TrafficCollectionConfig
    */
  def getOverallInterestingRequests: Int =
    getAppRequestAnalyses.map(_.getInterestingRequests.length).sum

  /** All RequestAnalyses that did not return in 'None'
    */
  def getAppRequestAnalyses: List[AppRequestTrackingEndpointAnalysis] =
    analysis.flatMap(_._2.map(_._2)).filter(_.nonEmpty).map(_.get).toList

  def getCoveredByEndpoint: Int = getAppRequestAnalyses.map(_.getEndpointParserCovered).sum

  /**
    */
  def getUnaddressed: Map[String, Map[String, Int]] = {
    val ret: MMap[String, MMap[String, Int]] = MMap()
    getAppRequestAnalyses.foreach { ana =>
      ana
        .getUnaddressedEndpoints
        .foreach { case (host, paths) =>
          ret.get(host) match {
            case Some(pathMap) =>
              paths.foreach { case (path, count) =>
                pathMap.get(path) match {
                  case Some(value) =>
                    pathMap(path) = value + count
                  case None =>
                    pathMap.addOne(path -> count)
                }
              }
            case None =>
  ...
  def getErrors: Map[String, List[String]] = {
    ...
    getAppRequestAnalyses.foreach { ana =>
      ana
        .getErrors
        .foreach { case (endpoint, errors) =>
          ret.get(endpoint) match {
            case Some(value) =>
              value.addAll(errors.map(_.value))
            case None =>
  ...
  /** generates a JsObject that can be pretty printed as specified below
    ...
    */
  def generateAnalysisSummary(analysis: List[AppRequestTrackingEndpointAnalysis]): JsObject = {
    ...
    analysis.foreach { ana =>
      ana
        .getPiiDistribution
        .foreach { case (pii, (anonCount, pseudoCount)) =>
    ...
    JsObject(
      ...
      "pii" ->
        JsObject(
          piiSummary
            .map { case (pii, (anon, pseudo)) =>
              pii -> JsObject("anonymous" -> JsNumber(anon), "pseudonymous" -> JsNumber(pseudo))
            }
            .toMap
        ),
      "endpoints" ->
...
object RequestTrackingEndpointAnalysis {
  ...
  def apply(experiment: Experiment, conf: TrafficCollectionAnalysisConfig, only: Set[String])(
    ...
    new RequestTrackingEndpointAnalysis(analysis, conf)
  }
