...
class RequestTrackingEndpointAnalysis(
  ...
  private val analysis: Map[MobileApp, List[
  ...
  def toJson: JsValue = {
    JsObject(
      ...
      "errors" -> JsObject(this.getErrors.filter(_._2.nonEmpty).map {
        case (endpoint, errors) =>
          endpoint -> JsArray(errors.map(value => JsString(value)).toVector)
      }),
      "apps" -> JsObject(
        this.analysis.map { case (app, analysis) =>
          app.toString -> JsObject(
            analysis.map { case (descr, analysis) =>
  ...
  def getUnaddressed: Map[String, Map[String, Int]] = {
    ...
    getAppRequestAnalysis.foreach { ana =>
      ana.getUnaddressedEndpoints.foreach { case (host, paths) =>
        ret.get(host) match {
          case Some(pathMap) =>
  ...
  def getErrors: Map[String, List[String]] = {
    ...
    getAppRequestAnalysis.foreach { ana =>
      ana.getErrors.foreach { case (endpoint, errors) =>
  ...
  def generateAnalysisSummary(
      analysis: List[AppRequestTrackingEndpointAnalysis]
  ): JsObject = {
    ...
    analysis.foreach { ana =>
      ana.getPiiDistribution.foreach { case (pii, (anonCount, pseudoCount)) =>
      ...
      ana.getCompanyPiiDistribution.foreach { case (company, piiMap) =>
    ...
    JsObject(
      ...
      "pii" -> JsObject(piiSummary.map { case (pii, (anon, pseudo)) =>
        pii -> JsObject(
      ...
      "endpoints" -> JsObject(
        endpointSummary.map { case (company, pii) =>
          company -> JsObject {
            pii.map { case (pii, (anon, pseudo)) =>
              pii -> JsObject(
